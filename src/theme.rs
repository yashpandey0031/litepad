...
pub fn apply(ctx: &egui::Context, pal: &Palette, dark: bool) {
    ...
    // Text-selection highlight inside the editor (kept subtle, readable in both themes).
    visuals.selection.bg_fill = if dark {
        Color32::from_rgb(0x33, 0x4A, 0x6B)
    } else {
        Color32::from_rgb(0xC7, 0xDB, 0xFF)
    };
